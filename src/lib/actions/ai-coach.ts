...
export async function askAiCoach(question: string) {
  ...
  const courseContext = (courses || [])
    ...
    .join("\n");
  const faqContext = (faqPosts || [])
    .map((p) => `Q: ${p.title}\nR: ${p.content?.slice(0, 200)}`)
    .join("\n");
...
Réponds de manière concise, actionnable et encourageante. Utilise le tutoiement. Max 300 mots.`;
  ...
  try {
    ...
    if (!answer) {
      console.warn(
        "[AI Coach] Réponse vide de OpenRouter, utilisation du fallback",
      );
      answer = getSmartFallback(question);
    }
  } catch (err) {
    console.error(
      "[AI Coach] Erreur appel IA:",
    ...
    if (errorMsg.includes("non configurée")) {
      answer =
        "⚠️ L'assistant IA n'est pas encore configuré. Demande à ton administrateur d'ajouter la clé API OpenRouter dans Paramètres > Intégrations.";
    } else {
      answer = getSmartFallback(question);
    }
  }
