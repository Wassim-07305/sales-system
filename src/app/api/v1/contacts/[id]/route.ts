...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const ALLOWED_FIELDS = ["full_name", "email", "phone", "company", "position", "notes", "status", "source", "tags"];
  ...
  const { data, error: dbError } = await supabase!.from("contacts").update(sanitized).eq("id", id).select().single();
