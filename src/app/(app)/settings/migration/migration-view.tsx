...
export function MigrationView({ history }: { history: MigrationLog[] }) {
  ...
  const [dataType, setDataType] = useState<"contacts" | "deals" | "both">("contacts");
  ...
  function handleExecute() {
    startTransition(async () => {
      try {
        ...
        const res = await executeMigration(contactRows, dealRows, config, fileName);
        setResult(res);
        setStep(3);
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {(Object.entries(CRM_PRESETS) as [CrmSource, typeof CRM_PRESETS.hubspot][]).map(
                  ([key, preset]) => (
                    <button
                      key={key}
                    ...
                    >
                      <Database className={cn("h-6 w-6 mx-auto mb-2", source === key ? "text-brand" : "text-muted-foreground")} />
              ...
              <div>
                <label className="text-sm font-medium mb-1.5 block">Type de données</label>
              ...
              <Button onClick={() => setStep(1)} className="bg-brand text-brand-dark hover:bg-brand/90">
                Continuer
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground mb-4">
              Exportez vos données depuis {CRM_PRESETS[source].label} au format CSV, puis importez-les ici.
              Les colonnes seront automatiquement mappées.
            </p>
            <div className="border-2 border-dashed rounded-lg p-8 text-center">
              <Upload className="h-8 w-8 mx-auto mb-3 text-muted-foreground" />
              <p className="text-sm font-medium mb-2">Glissez un fichier CSV ou cliquez pour sélectionner</p>
              <Input
                type="file"
                accept=".csv"
                    ...
                    <thead>
                      <tr className="border-b bg-muted/50">
                        {csvHeaders.slice(0, 6).map((h) => (
                          <th key={h} className="text-left p-2 font-medium truncate max-w-[120px]">{h}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
                      {csvRows.slice(0, 3).map((row, i) => (
                        <tr key={i} className="border-b">
                          {csvHeaders.slice(0, 6).map((h) => (
                            <td key={h} className="p-2 truncate max-w-[120px] text-muted-foreground">{row[h]}</td>
                          ))}
                        </tr>
                      ))}
                    </tbody>
