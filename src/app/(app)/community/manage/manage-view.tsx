...
import { hidePost, unhidePost, deleteCommunityPost } from "@/lib/actions/community";
...
export function ManageView({ posts }: { posts: Post[] }) {
  ...
  return (
    <div>
      <PageHeader title="Modération" description={`${posts.length} posts au total`}>
        <Link href="/community">
          ...
          <Card key={post.id} className={post.hidden ? "opacity-50" : ""}>
            <CardContent className="p-4 flex items-start justify-between gap-4">
              ...
              <div className="flex gap-1 shrink-0">
                {post.hidden ? (
                  <Button size="sm" variant="ghost" onClick={() => handleUnhide(post.id)}><Eye className="h-4 w-4" /></Button>
                ) : (
                  <Button size="sm" variant="ghost" onClick={() => handleHide(post.id)}><EyeOff className="h-4 w-4" /></Button>
                )}
