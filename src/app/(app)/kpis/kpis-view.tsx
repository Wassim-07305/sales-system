...
export function KpisView({
  ...
  // Compute stats
  const totalBookings = filteredKpis.reduce(
    (sum, k) => sum + k.bookings_count,
    0,
  );
  const avgShowUp =
    filteredKpis.length > 0
      ? Math.round(
          filteredKpis.reduce((sum, k) => sum + Number(k.show_up_rate), 0) /
            filteredKpis.length,
        )
      : 0;
  const avgClosing =
    filteredKpis.length > 0
      ? Math.round(
          filteredKpis.reduce((sum, k) => sum + Number(k.closing_rate), 0) /
            filteredKpis.length,
        )
      : 0;
  const totalRevenue = filteredKpis.reduce(
    (sum, k) => sum + Number(k.revenue_signed),
    0,
  );

  const chartData = filteredKpis.map((k) => ({
    date: new Date(k.date).toLocaleDateString("fr-FR", {
      day: "numeric",
      month: "short",
    }),
