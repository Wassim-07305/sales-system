...
export async function calculateReadinessScore(
  ...
  for (const attempt of quizAttempts || []) {
    if (
      !bestByQuiz.has(attempt.quiz_id) ||
      attempt.score > bestByQuiz.get(attempt.quiz_id)!.score
    ) {
      bestByQuiz.set(attempt.quiz_id, {
        score: attempt.score,
  ...
  const averageQuizScore =
    bestByQuiz.size > 0
      ? Math.round(
          [...bestByQuiz.values()].reduce((sum, q) => sum + q.score, 0) /
            bestByQuiz.size,
        )
      : 0;
  ...
  const scoreFactor =
    averageRoleplayScore >= 70 ? 1 : averageRoleplayScore >= 50 ? 0.8 : 0.6;
...
export async function updateReadinessStatus(
  ...
  if (readiness.isReady && !wasReady) {
    ...
    if (adminIds.length > 0) {
      await notifyMany(
        adminIds,
        "Setter pret a etre place !",
        `${profile?.full_name || "Un setter"} a atteint ${readiness.overall}% de preparation et est pret a etre place aupres d'un entrepreneur.`,
        { type: "placement_ready", link: "/team" },
      );
    }
    ...
    await notify(
      userId,
      "Felicitations ! Tu es pret a etre place !",
      `Tu as atteint ${readiness.overall}% de preparation. L'equipe va te contacter pour ton placement aupres d'un entrepreneur.`,
      { type: "placement_ready", link: "/dashboard" },
    );
  }
